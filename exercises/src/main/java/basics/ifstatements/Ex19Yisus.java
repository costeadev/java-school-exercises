...
public class Ex19Yisus {

    //Recibe tres letras minúsculas y un number entero. Sustituye cada char
    ...
    public static void main(String[] args) {
        //Declaramos y leemos los valuees de las variables de entrada
        ...
        System.out.println("Introduce un number entero positivo");
        ...
        if (c1 < 97 || c1 > 122 || c2 < 97 || c2 > 122 || c3 < 97 || c3 > 122) {
            ...
            //Mostramos un mensaje de error si el number es negativo
        } else if (n < 0) {
            System.out.println("El number introducido no puede ser negativo");
            //Limito para el resultado de la suma del number introducido más el value del caracter no exceda el value de la capacidad de un int,
            //que parece ser que causa que el number se vuelva negativo tras exceder ese value
        } else if (n > (Integer.MAX_VALUE - 122)) {
            System.out.println("El number introducido es demasiado grande");

            //Si los caracteres introducidos son válidos, entonces sumamos el value decimal del caracter ASCII al number introducido por teclado.
            ...
            //Si la suma del value decimal del caracter introducido y el number introducido es mayor que 122 (que es el value de la z, la última letra minúscula de la tabla ASCII),
            ...
            //Estas 3 líneas de código de debajo tienen la única función de saber el value de cNx en el caso de que se esté depurando
            //el código y queramos ver el value de cNx para poder hacer nosotros cálculos manuales en un papel
            //System.out.println("test. value cN1:  " + cN1);
            //System.out.println("test. value cN2:  " + cN2);
            //System.out.println("test. value cN3:  " + cN3);

            //Bloques de condicionales para resolver el primer caracter

            //Si el number introducido es igual a 122 o uno de sus múltiplos, se dará una vuelta completa al abecedario. Por tanto, el value final del caracter (cNx), seguirá siendo el mismo (cNx == cx).
            if (cN1 == c1) {
                System.out.println("La primera letra introducida no cambia, sigue siendo letra 1: " + (char) cN1);

                //Hace falta meter este caso de debajo, porque si se da por ejemplo un caso de que el caracter fuera una y (y == 121) y el number introducido es n = 123, entonces el value de cNx sería 0, y por tanto
                ...
                //La forma de resolver esto es poner en este else if la condición de que cNx == 0 y que el value inicial del caracter introducido (c1) no fuera la z (122). Si el caracter introducido era la z, entonces
                //salta al siguiente else if
            } else if (cN1 == 0 && c1 != 122) {
                cN1 = c1;
                System.out.println("La primera letra introducida no cambia, sigue siendo letra 1: " + (char) cN1);

                //Este else if hace falta meterlo porque si n es igual a cualquier múltiplo de 122 y el caracter introducido es una z (122 en ASCII), el value de cNx sería 0 (el resto de
                ...
                //lo que en realidad se ha hecho es dar una vuelta completa al abecedario de minúsculas, y el caracter debería seguir siendo z. Por tanto, al value del caracter final le asignamos el 122
                ...
                //en el else if anterior, ya se demostró que el value inicial del caracter cx era igual a 122)
            } else if (cN1 == 0) {
                cN1 = 122;
                System.out.println("La primera letra introducida no cambia, sigue siendo letra 1: " + (char) cN1);

                //Es bastante probable que se de el caso de que cNx sea mayor que 122, y por tanto al cambiar el value de cNx por cNx % 122, el value sea menor a 97 (que es el value de la primera
                //letra minúsucla, a, en la tabla ASCII). Si se da este caso e imprimimos directamente este value, nos daría un caracter en la tabla ASCII que está a la izquierda de donde empiezan
                //las letras minúsculas. Por tanto, la solución es sumar el value de cNx a 96, que es el caracter que está justo antes de la a (pues al terminar el abecedario, se avanza el number
                //de posiciones que toque desde la z. La forma de emular esto es usar el value del caracter justo antes de la a). Así, sabemos cuántas posiciones ha avanzado desde que llegó al final
                ...
                //Si restamos 122 al value de cNx, sabemos cuántas posiciones ha avanzado desde el final (122). El resultante de eso, se lo
                //sumamos al value del caracter inmediatamente anterior al comienzo del abecedario de minúsculas (' == 96), como ya se explicó en
                ...
                //Así, resolvemos el problema aunque el value de cNx sea muy grande.
                if (cN1 > 122) {
                    cN1 = cN1 - 122 + 96;
                }
                if (cN1 > 122) {
                ...
                //Este sería el caso en el que no se llega a dar una vuelta completa, dado que la suma del value del caracter introducido y del number n introducido no llega a dar una vuelta completa
                //(es menor a 122, que es el value de la z). Por tanto, nos quedamos con ese value tal cual y lo imprimimos como char
            } else {
                System.out.println("La primera letra introducida cambia a: " + (char) cN1);
            }
